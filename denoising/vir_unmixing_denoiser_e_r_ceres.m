function [X,d,c,C,Z,Xlib,X_BB,bp] = vir_unmixing_denoiser_e_r_ceres(Alib,BB,Y,wv,varargin)
% [X,d,c,C,Z,Xlib,X_BB,bp] = vir_unmixing_denoiser_e_r_ceres(Alib,BB,Y,wv,varargin)
...
%  Ceres data processing where I expect relatively accurate initial input 
%  of d0, so the threshold values are set small.
%  if the standard deviation is larger than 0.0006, then the band is
%  considered as bad. If the residual (from its median) is greater than 5 sigma, then that is
%  considered as a temporal spike. 5 sigma is quite conservative.
% 
%  INPUTS
...
%     [X,Z,C,r,dd,rho,Rhov,res_p,res_d] = huwacbl1_cbp_gadmm(A,Ycd,wv,...
%          'LAMBDA_A',lambda_a,'ConcaveBase',C,'tol',1e-5,'maxiter',1000,'verbose','yes');
    ...
    if i>0
        ...
        bp_singles = rr_median_out > 5*bp_std;
